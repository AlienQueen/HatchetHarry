...
public class SpringContextLoaderBaseTest {
	...
	public static void setUpBeforeClass() throws IOException {
		...
		SpringContextLoaderBaseTest.waTester = new AtmosphereTester(SpringContextLoaderBaseTest.tester, new HomePage());
	}
	...
	public static Long startAGameAndPlayACard(final WicketTester _tester,
		...
		Player p = SpringContextLoaderBaseTest.persistenceService.getAllPlayersOfGame(HatchetHarrySession.get().getGameId()).get(0);
		...
		final WebMarkupContainer playCardLink = (WebMarkupContainer) _tester.getComponentFromLastRenderedPage("playCardLink");
		final PlayCardFromHandBehavior pcfhb = (PlayCardFromHandBehavior) playCardLink.getBehaviors().get(0);
		...
		// Play a card
		_tester.getRequest().setParameter("card", HatchetHarrySession.get().getFirstCardsInHand().get(0).getUuid());
...
//		_tester.assertRenderedPage(HomePage.class);
		...
		// deck
		p = SpringContextLoaderBaseTest.persistenceService.getAllPlayersOfGame(HatchetHarrySession.get().getGameId()).get(0);
		Assert.assertEquals(60, p.getDeck().getCards().size());
