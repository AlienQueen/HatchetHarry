...
{
	/**
	 * Init: we create a game, we play a card, we tap it, we put it back to hand Run: we play it
	 * again Verify: the card should be untapped.
	 *
	 */
	@Test
		...
		throws Exception
	{
		...
		allCardsInBattlefield = SpringContextLoaderBase.persistenceService
			.getAllCardsAndTokensInBattlefieldForAGameAndAPlayer(gameId, HatchetHarrySession.get()
				.getPlayer().getId(), HatchetHarrySession.get().getPlayer().getDeck().getDeckId());
		...
		throws Exception
	{
		...
		List<MagicCard> cardsOnBattlefield = SpringContextLoaderBase.persistenceService
			.getAllCardsAndTokensInBattlefieldForAGameAndAPlayer(p.getGame().getId(), p.getId(), p
				.getDeck().getDeckId());
		...
		throws Exception
	{
		...
		// Get 3rd card name
		final Player p = SpringContextLoaderBase.persistenceService.getPlayer(HatchetHarrySession
	...
	public void testDiscardingAcardAtRandomShouldRemoveACardFromHand() throws Exception
	{
		...
		final List<MagicCard> allCardsInGraveyard = persistenceService
			.getAllCardsInGraveyardForAGameAndAPlayer(gameId, HatchetHarrySession.get().getPlayer()
		...
		Assert.assertEquals(
			...
			persistenceService.getAllCardsFromDeck(
				HatchetHarrySession.get().getPlayer().getDeck().getDeckArchive().getDeckName())
				.size());
	}
