...
public class RuntimeDataGenerator implements Serializable
{
	...
	public Deck generateData(final Long gameId, final Long playerId) throws IOException
	{
		...
		RuntimeDataGenerator.LOGGER.info("preparing to import decks");
		...
		{
			RuntimeDataGenerator.LOGGER.info("importing decks");
			...
			for (int j = 1; j < 3; j++)
			{
				for (int i = 0; i < 60; i++)
				{
					...
					cc.setTitle(RuntimeDataGenerator.TITLES1[i]);
					...
					if (j == 1)
					{
						card = new MagicCard("cards/" + RuntimeDataGenerator.TITLES1[i]
					...
					{
						card = new MagicCard("cards/" + RuntimeDataGenerator.TITLES2[i]
							+ "_small.jpg", "cards/" + RuntimeDataGenerator.TITLES2[i] + ".jpg",
							"cards/" + RuntimeDataGenerator.TITLES2[i] + "Thumb.jpg",
				...
				if (j == 1)
				{
					...
					RuntimeDataGenerator.LOGGER.info("updating deck 1");
				}
				else
				{
					this.persistenceService.updateDeckArchive(da2);
					this.persistenceService.updateDeck(deck2);
					RuntimeDataGenerator.LOGGER.info("updating deck 2");
				}
			}
			return deck1;
		}
