...
public class PersistenceService implements Serializable {
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED, isolation = Isolation.READ_COMMITTED)
    public MagicCard saveTokenAndDeck(final MagicCard c) {
        c.setDeck((Deck) this.deckDao.getSession().merge(c.getDeck()));
        c.getToken().setPlayer((Player) this.playerDao.getSession().merge(c.getToken().getPlayer()));
		this.tokenDao.getSession().save(c.getToken());
        return c;
    }
