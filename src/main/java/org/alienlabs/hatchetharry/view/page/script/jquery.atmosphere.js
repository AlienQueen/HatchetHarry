 ...
 * jQuery Stream
 * 
 * @VERSION Comet Streaming JavaScript Library
 *          http://code.google.com/p/jquery-stream/
 * 
 * Copyright 2011, Donghwan Kim Licensed under the Apache License, Version 2.0
 * http://www.apache.org/licenses/LICENSE-2.0
 * 
...
jQuery.atmosphere = function() {
	...
	return {
		...
		jsonp : function() {
			...
			var jqxhr = jQuery.ajax({
				...
				error : function(jqXHR, textStatus, errorThrown) {
					jQuery.atmosphere.ieCallback(textStatus, "error",
							jqXHR.status, request.transport);
				},
				...
				success : function(json) {
					...
					jQuery.atmosphere.ieCallback(msg, "messageReceived", 200,
							request.transport);
		...
		executeRequest : function() {
			...
			if (jQuery.atmosphere.request.transport == 'jsonp'
					|| (jQuery.atmosphere.request.enableXDR && jQuery.atmosphere
							.checkCORSSupport())) {
			...
			if (jQuery.atmosphere.request.transport == 'streaming'
					&& jQuery.browser.msie) {
				jQuery.atmosphere.request.enableXDR && window.XDomainRequest ? jQuery.atmosphere
						.ieXDR()
			...
			if (jQuery.atmosphere.request.requestCount++ < jQuery.atmosphere.request.maxRequest) {
				...
				ajaxRequest.onreadystatechange = function() {
					if (jQuery.atmosphere.abordingConnection)
						return;
					...
					if (request.transport == 'streaming'
							&& (request.readyState > 2 && ajaxRequest.readyState == 4)) {

						request.readyState = 0;
						request.lastIndex = 0;

						jQuery.atmosphere.reconnect(ajaxRequest,
								jQuery.atmosphere.request, true);
						return;
					}
					...
					if (update) {
						...
						if (request.transport == 'streaming') {
							response.responseBody = responseText.substring(
									request.lastIndex, responseText.length);
							response.isJunkEnded = true;

							if (request.lastIndex == 0
									&& response.responseBody
											.indexOf("<!-- Welcome to the Atmosphere Framework.") != -1) {
							...
							if (!response.isJunkEnded) {
								...
								var junkEnd = response.responseBody
										.indexOf(endOfJunk)
										+ endOfJunkLenght;

								if (junkEnd > endOfJunkLenght
										&& junkEnd != response.responseBody.length) {
										...
										.iterate(
												function() {
													if (ajaxRequest.responseText.length > request.lastIndex) {
														...
														if ((request.transport == 'streaming')
																&& (ajaxRequest.responseText.length > jQuery.atmosphere.request.maxStreamingLength)) {
															// Close and reopen
															// connection on
															// large data
															// received
						...
						if (response.responseBody.indexOf("parent.callback") != -1) {
							jQuery.atmosphere
									.log(
											logLevel,
											[ "parent.callback no longer supported with 0.8 version and up. Please upgrade" ]);
						}
						jQuery.atmosphere.invokeCallback(response);
						...
						if ((request.transport == 'streaming')
								&& (responseText.length > jQuery.atmosphere.request.maxStreamingLength)) {
							// Close and reopen connection on large data
							// received
							ajaxRequest.abort();
		...
		doRequest : function(ajaxRequest, request, create) {
			...
			ajaxRequest.setRequestHeader("X-Atmosphere-tracking-id",
					jQuery.atmosphere.uuid);
		...
		reconnect : function(ajaxRequest, request, force) {
			...
			if (force
					|| (request.suspend && ajaxRequest.status == 200 && request.transport != 'streaming')) {
		...
		configureIE : function() {
			...
			return {
				open : function() {
					...
					// Finally attach a timestamp to prevent Android and IE
					// caching.
					...
					// For the server to respond in a consistent format
					// regardless of user agent, we polls response text
					var cdoc = iframe.contentDocument
							|| iframe.contentWindow.document;

					stop = jQuery.atmosphere
							.iterate(function() {
								if (!cdoc.firstChild) {
									return;
								}
								...
								if (cdoc.readyState === "complete") {
									try {
										...
										jQuery.atmosphere.ieCallback(
												"Connection Failure", "error",
												500, request.transport);
										return false;
									}
								}

								var res = cdoc.body ? cdoc.body.lastChild
										: cdoc, readResponse = function() {
									// Clones the element not to disturb the
									// original one
									var clone = res.cloneNode(true);

									// If the last character is a carriage
									// return or a line feed, IE ignores it in
									// the innerText property
									// therefore, we add another non-newline
									...
									if (isJunkEnded) {
										var endOfJunk = "<!-- EOD -->";
										var endOfJunkLenght = endOfJunk.length;
										var junkEnd = text.indexOf(endOfJunk)
												+ endOfJunkLenght;
								...
								if (!jQuery.nodeName(res, "pre")) {
									// Injects a plaintext element which renders
									// text without interpreting the HTML and
									// cannot be stopped
									// it is deprecated in HTML5, but still
									// works
									var head = cdoc.head
											|| cdoc
													.getElementsByTagName("head")[0]
											|| cdoc.documentElement || cdoc, script = cdoc
									...
									// The plaintext element will be the
									// response container
									res = cdoc.body.lastChild;
								}

								// Handles open event
								jQuery.atmosphere.ieCallback(readResponse(),
										"messageReceived", 200,
										request.transport);
		...
		configureXDR : function() {
			...
			var xdr = new window.XDomainRequest(), rewriteURL = jQuery.atmosphere.request.rewriteURL
					|| function(url) {
			...
			return {
				open : function() {
					var url = jQuery.atmosphere
							.attachHeaders(jQuery.atmosphere.request);
		...
		executeWebSocket : function() {
					...
					jQuery.atmosphere
							.log(
									...
									[ "Websocket failed. Downgrading to Comet and resending "
											+ data ]);
					// Websocket is not supported, reconnect using the fallback
					// transport.
			...
			websocket.onerror = function(message) {
				jQuery.atmosphere.warn("Websocket error, reason: "
						+ message.reason);
			...
			websocket.onclose = function(message) {
				...
				jQuery.atmosphere.log(logLevel, [ "Websocket closed, reason: "
						+ reason ]);
				jQuery.atmosphere.log(logLevel,
						[ "Websocket closed, wasClean: " + message.wasClean ]);
				...
				if (!webSocketSupported) {
					...
					if (request.requestCount++ < request.maxRequest) {
						...
						jQuery.atmosphere.request.url = jQuery.atmosphere
		...
		guid : function() {
			return (jQuery.atmosphere.S4() + jQuery.atmosphere.S4() + "-"
					+ jQuery.atmosphere.S4() + "-" + jQuery.atmosphere.S4()
					+ "-" + jQuery.atmosphere.S4() + "-"
					+ jQuery.atmosphere.S4() + jQuery.atmosphere.S4() + jQuery.atmosphere
					.S4());
		},
		...
		prepareURL : function(url) {
			...
			var ts = jQuery.now(), ret = url.replace(/([?&])_=[^&]*/, "$1_="
					+ ts);

			return ret
					+ (ret === url ? (/\?/.test(url) ? "&" : "?") + "_=" + ts
							: "");
		},
		...
		iterate : function(fn, interval) {
			...
			// Though the interval is 0 for real-time application, there is a
			// delay between setTimeout calls
			// For detail, see
				...
				while ((pos = paramsStr.search(/\&/)) >= 0) {
					...
					if (paramStr.length) {
						...
						if (equPos < 0) {
							...
							parameters[paramStr.substring(0, equPos)] = decodeURIComponent(paramStr
									.substring(equPos + 1));
						}
					}
				}
...
/*
 * jQuery stringifyJSON http://github.com/flowersinthesand/jquery-stringifyJSON
 * 
 * Copyright 2011, Donghwan Kim Licensed under the Apache License, Version 2.0
 * http://www.apache.org/licenses/LICENSE-2.0
 */
