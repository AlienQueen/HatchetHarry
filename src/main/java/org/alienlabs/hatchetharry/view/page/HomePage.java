...
public class HomePage extends TestReportPage
{
	...
	public HomePage() throws IOException
	{
		...
		final Label message2 = new Label("message2", "built on Tuesday, 15th of July 2014.");
		...
		for (final CardPanel cp : allCards)
		{
			try
			{
				...
				if (null != mc)
				{
					...
					if (mc.getCounters().isEmpty())
					{
						js.append("jQuery('#bullet" + cp.getUuid().toString().replace("-", "_")
								+ "').hide(); ");
					}
					else
					{
