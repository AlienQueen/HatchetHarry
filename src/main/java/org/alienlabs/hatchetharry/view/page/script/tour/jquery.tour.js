...
(function($) {
	...
	var Tour = {
		...
		showTooltip : function() {
			...
			if ($.tour.options.autoplay) {
				$.tour.options.showtime = setTimeout(Tour.next(),
						step_config.time);
			}
			...
			/*
			 * if the element is not in the viewport we scroll to it before
			...
			if ((b_t < w_t || b_t > w_b) || (b_b < w_t || b_b > w_b)) {
				$('html, body').stop().animate(
						...
						'easeInOutExpo',
						function() {
							// need to reset the timeout because of the
							// animation delay
							if ($.tour.options.autoplay) {
								clearTimeout($.tour.options.showtime);
								$.tour.options.showtime = setTimeout(Tour
										.next(), step_config.time);
							}
