...
public class JavaScriptUtils
{
	...
	public static final String REVEAL_HAND_COMPONENT_JAVASCRIPT = "var theIntPlayer = null; var $crosslinkPlayer, $navthumbPlayer; var curclickedPlayer = 0; theIntervalPlayer = function(cur) { if (typeof cur != 'undefined') curclickedPlayer = cur; $crosslinkPlayer.removeClass('active-thumb-Hand'); $navthumbPlayer.eq(curclickedPlayer).parent().addClass('active-thumb-Hand'); jQuery('.stripNavPlayer ul li a').eq(curclickedPlayer).trigger('click'); $crosslinkPlayer.removeClass('active-thumb-Hand'); $navthumbPlayer.eq(curclickedPlayer).parent().addClass('active-thumb-Hand'); jQuery('.stripNavPlayer ul li a').eq(curclickedPlayer).trigger('click'); curclickedPlayer++; if (6 == curclickedPlayer) curclickedPlayer = 0; }; jQuery('#main-photo-sliderPlayer').codaSliderPlayer(); $navthumbPlayer = jQuery('.nav-thumb'); $crosslinkPlayer = jQuery('.cross-link'); $navthumbPlayer.click(function() { var $thisPlayer = jQuery(this); theIntervalPlayer($thisPlayer.parent().attr('href').slice(1) - 1); return false; }); theIntervalPlayer(); function updateHandLabelPlayer(){ var handCardNamePlayer = jQuery('#handGalleryPlayer .active-thumb-Hand .nav-thumb').attr('name'); if (handCardNamePlayer == undefined) { handCardNamePlayer = jQuery('#handGalleryPlayer .active-thumb-Hand .nav-thumb').attr('name'); } jQuery('#cardLabel').text(handCardNamePlayer); }; jQuery(function() { setTimeout(function() { updateHandLabelPlayer(); jQuery('#handGalleryPlayer .cross-link .nav-thumb').click(updateHandLabelPlayer); jQuery('#stripNavPlayer0').hide(); jQuery('#galleryRevealParent').children().first().attr('style', ''); jQuery('#handGalleryPlayer').attr('style', 'width: 262px; margin: 25px auto; position: absolute; top: 2%; left: 50%; min-height: 420px;'); jQuery('#handlehandGalleryPlayer').attr('style', 'position: relative; top: 110px; left: 0%; z-index: 10; cursor: move;'); jQuery('#handGalleryPlayer').draggable({ handle : '#handlehandGalleryPlayer', helper : 'original'}); }, 500); }); ";
