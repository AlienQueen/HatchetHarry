...
public class JavaScriptUtils
{
	...
	public static final String REACTIVATE_HAND_COMPONENT_JAVASCRIPT = "window.setTimeout(function() { sortableInProgress = false; function turn(){ if (sortableInProgress) { sortableInProgress = false; return; } $(this).parent().toggleClass('turned'); } $('.cards').sortable({ placeholder: 'ui-state-highlight', start: function( event, ui ) { sortableInProgress = true; } }); $('.magicCard').click(tooltips); function tooltips(){ $(this).parent().toggleClass('details'); if($(this).parent().hasClass('details')) $('img',$(this).parent()).unbind('click').click(tooltips); else $('img',$(this).parent()).unbind('click').click(turn); } }, 1000); ";
