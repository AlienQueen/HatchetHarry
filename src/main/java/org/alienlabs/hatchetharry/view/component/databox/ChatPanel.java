...
public class ChatPanel extends Panel
{
	...
	String user;
	String message;
	AjaxButton sendMessage;

	public ChatPanel(final String id, final IPushServiceRef<?> pushServiceRef)
	{
		super(id);

		final String chatRoomName = "#chat";
		final UpdateDataBoxChannel updateDataBoxChannel = UpdateDataBoxServiceLocator
				.getChatService().getChatRoom(chatRoomName);
		...
		// user field
		formChat.add(new RequiredTextField<String>("user"));

		// message field
		final TextField<String> messageField = new RequiredTextField<String>("message");
		messageField.setOutputMarkupId(true);
		formChat.add(messageField);

		// send button
		formChat.add(this.sendMessage = new AjaxButton("send", formChat)
		{
			private static final long serialVersionUID = 1L;
			...
			protected void onSubmit(final AjaxRequestTarget target, final Form<?> form)
			{
				...
				UpdateDataBoxServiceLocator.getChatService().getChatRoom(chatRoomName)
						.sendAsync(ChatPanel.this.user, ChatPanel.this.message);
		 ...
		 * install push node
		 */
		final IPushNode<UpdateDataBoxMessage> pushNode = pushServiceRef.get().installNode(this,
				new AbstractPushEventHandler<UpdateDataBoxMessage>()
				{
					private static final long serialVersionUID = 1L;

					@Override
					public void onEvent(final AjaxRequestTarget target,
							final UpdateDataBoxMessage _message,
							final IPushNode<UpdateDataBoxMessage> node,
							final IPushEventContext<UpdateDataBoxMessage> ctx)
					{
						this.appendHTML(target, chatHistoryField,
								ChatPanel.this._renderMessage(_message));
					}
				});
		...
		formChat.add(new AjaxButton("disconnect", formChat)
		{
			...
			protected void onSubmit(final AjaxRequestTarget target, final Form<?> form)
			{
				pushServiceRef.get().uninstallNode(ChatPanel.this, pushNode);
				target.add(this.setEnabled(false));
				target.add(ChatPanel.this.sendMessage.setEnabled(false));
			}
		});
		 ...
		 */
		updateDataBoxChannel.addListener(new IUpdateDataBoxListener()
		{
			private static final long serialVersionUID = 1L;

			@Override
			public void onMessage(final UpdateDataBoxMessage msg)
			{
				...
				{
					updateDataBoxChannel.removeListener(this);
				}
			}
		});
		...
		// for (final UpdateDataBoxMessage msg :
		// updateDataBoxChannel.getChatHistory())
		// {
		...
		pushServiceRef.get().addNodeDisconnectedListener(new IPushNodeDisconnectedListener()
		{
			...
			public void onDisconnect(final IPushNode<?> node)
			{
				...
				{
					// chatRoom.sendAsync("<System>",
					// "A USER JUST LEFT THE ROOM.");
	...
	String _renderMessage(final UpdateDataBoxMessage msg)
	{
