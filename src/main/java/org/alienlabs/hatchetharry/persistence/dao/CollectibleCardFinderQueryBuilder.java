 ...
 */
public class CollectibleCardFinderQueryBuilder
{
	private List<String> parameters;
	private List<AbstractSingleColumnStandardBasicType<?>> types;
	private boolean count;
	private CollectibleCard filter = new CollectibleCard();
	...
	public String buildHql()
	{
		...
		hql.append("from CollectibleCard target where 1=1 ");
		if (this.filter.getTitle() != null)
		{
			this.addMatchingCondition(hql, this.filter.getTitle().toString(), "title");
		}
		else
		{
	...
	public void setFilter(final CollectibleCard _filter)
	{
		if (_filter == null)
		{
