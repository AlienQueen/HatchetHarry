 ...
 */
public class PlayerFinderQueryBuilder
{
	...
	public String buildHql()
	{
		...
		hql.append("from Player target where 1=1 ");
		if (this.filter.getName() != null)
		{
			this.addMatchingCondition(hql, this.filter.getName().toString(), "name");
		}
		else
		{
			this.addMatchingCondition(hql, "*", "name");
		}
		this.addMatchingCondition(hql, this.filter.getSide(), "side");
	...
	public void setFilter(final Player _filter)
	{
		if (_filter == null)
		{
