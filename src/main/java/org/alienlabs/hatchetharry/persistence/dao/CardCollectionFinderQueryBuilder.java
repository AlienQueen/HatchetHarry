 ...
 */
public class CardCollectionFinderQueryBuilder
{
	private List<String> parameters;
	private List<AbstractSingleColumnStandardBasicType<?>> types;
	private boolean count;
	private CardCollection filter = new CardCollection();
	...
	public String buildHql()
	{
		...
		hql.append("from CardCollection target where 1=1 ");
		if (this.filter.getName() != null)
		{
			this.addMatchingCondition(hql, this.filter.getName().toString(), "title");
		}
		else
		{
			this.addMatchingCondition(hql, "*", "id");
		}
	...
	public void setFilter(final CardCollection _filter)
	{
		if (_filter == null)
		{
